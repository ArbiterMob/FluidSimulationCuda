// CUDA first version naive (1 thread for each cell)
...
__global__ void diffuseOnGPU(int b, float *d_x, float *d_x0, float *d_xTemp, float alpha, float beta) {
    ...
    for (int k = 0; k < 40; k++) {
        if (ix >= 1 && ix <= N && iy >= 1 && iy <= N) {
            ...
            //    d_xTemp[tid], ix, iy, d_x0[tid], d_x[tid - 1], d_x[tid + 1], d_x[tid - N - 2], d_x[tid + N + 2], beta);
        }
...
void dens_step(dim3 grid, dim3 block, float *d_x, float *d_x0, float *d_u, float *d_v, float *d_densTemp) {
    ...
    //cudaDeviceSynchronize(); // to ensure correct computation
    ...
    //for (int k = 0; k < 40; k++) { // inefficient -> multiple kernel calls
        ...
        //cudaDeviceSynchronize(); // to ensure correct computation BUT inefficent
        SWAP(d_densTemp, d_x);
    //}
