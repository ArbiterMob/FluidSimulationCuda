...
__device__ void set_bndOnGPU(int b, float *x) {
    int ix = threadIdx.x + blockDim.x * blockIdx.x;
    int iy = threadIdx.y + blockDim.y * blockIdx.y;
    int idx = ix + iy * (N + 2);

    if (iy == 0)
        x[idx] = b == 1 ? -x[idx + N + 2] : x[idx + N + 2];
    if (iy == N + 1)
        x[idx] = b == 1 ? -x[idx - N - 2] : x[idx - N - 2];
    if (ix == 0)
        x[idx] = b == 2 ? -x[idx + 1] : x[idx + 1];
    if (ix == N + 1)
        x[idx] = b == 2 ? -x[idx - 1] : x[idx - 1];

    if (ix == 0 && iy == 0)
        x[idx] = 0.5f * (x[idx + 1] + x[idx + N + 2]);
    if (ix == N + 1 && iy == 0)
        x[idx] = 0.5f * (x[idx - 1] + x[idx + N + 2]);
    if (ix == 0 && iy == N + 1)
        x[idx] = 0.5f * (x[idx + 1] + x[idx - N - 2]);
...
__global__ void add_sourceOnGPU(float *d_x, float *d_s) {
    ...
    int section_sizeX = (sizeX - 1) / (blockDim.x * gridDim.x) +  1;
    int section_sizeY = (sizeY - 1) / (blockDim.y * gridDim.y) + 1;
    int startX = ix * section_sizeX;
    int startY = iy * section_sizeY;

    int idx = ix + iy * (N + 2);

    for (int j = 0; j < section_sizeY; j++) {
        for (int i = 0; i < section_sizeX; i++) {
            if(startX + i < sizeX && startY + j < sizeY)
                d_x[i + j * (N + 2)] += DT * d_s[i + j * (N + 2)];
        }
    }
...
__global__ void diffuseOnGPU(int b, float *d_x, float *d_x0, float *d_x_new, int whichAlpha) {
    ...
    if (ix >= 1 && iy >= 1 && ix <= N && iy <= N) {
        ...
        d_x_new[idx] = (d_x0[idx] + a * (d_x[left] + d_x[right] + d_x[bot] + d_x[top])) / (1 + 4 * a);
...
void dens_step(dim3 grid, dim3 block, float *d_x, float *d_x0, float *d_u, float *d_v, float *d_dens_new) {
    ...
    SWAP(d_x0, d_x);
    for (int k = 0; k < 40; k++) {
        diffuseOnGPU<<<grid, block>>>(0, d_x, d_x0, d_dens_new, 0);
        cudaDeviceSynchronize();
...
void vel_step(dim3 grid, dim3 block, float *d_u, float *d_v, float *d_u0, float *d_v0, float *d_u_new, float *d_v_new) {
    ...
    for (int k = 0; k < 40; k++) {
        diffuseOnGPU<<<grid, block>>>(1, d_u, d_u0, d_u_new, 0);
        diffuseOnGPU<<<grid, block>>>(2, d_v, d_v0, d_v_new, 0); 
        cudaDeviceSynchronize();
    ...
    //cudaDeviceSynchronize();
    for (int k = 0; k < 40; k++) {
        diffuseOnGPU<<<grid, block>>>(0, d_u0, d_v0, d_u_new, 1);
        cudaDeviceSynchronize();
        //set_bndOnGPU<<<grid, block>>>(0, d_u0);
        //cudaDeviceSynchronize();
        SWAP(d_u0, d_u_new)
...
void initializeParameters(float *dens, float *dens_prev, float *u, float *u_prev, float *v, float *v_prev, int hN) {
    ...
    for (i = 0; i < hN + 2; i ++) {
        for (j = 0; j < hN + 2; j++) {
            ...
            if (dist < radius) 
                dens_prev[j + i * (hN + 2)] = (1.0f - (dist / radius)) * 10;
            else 
    ...
    for (i = 0; i < hN + 2; i ++) {
        for (j = 0; j < hN + 2; j++) {
            u_prev[j + i * (hN + 2)] = ((rand() % 100) / 100.0f) * 0.1f; // Random values between 1.0 and 10.0
            v_prev[j + i * (hN + 2)] = ((rand() % 100) / 100.0f) * 0.1f;// Random values between 1.0 and 10.0
...
int main(int argc, char **argv) {
    ...
    */
    ...
    grid = dim3((hN + block.x - 1) / block.x, (hN + block.y - 1) / block.y);
