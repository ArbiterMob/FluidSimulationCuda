...
__global__ void add_sourceOnGPU(float *d_x, float *d_s) {
    ...
    for (i = 0; i < section_size_y; i++) {
        for (j = 0; j < section_size_x; j++) {
            ...
            //printf("lTid %d - sTid %d| nIX %d - nIY %d | nTid %d | d_x_s[lTid] %f | d_x[nTid] %f | d_s_s[lTid] %f | d_s[nTid] %f\n",
            //    lTid, sTid, nIX, nIY, nTid, d_x_s[sTid], d_x[nTid], d_s_s[sTid], d_s[nTid]);
        }
    }
    ...
    for (i = 0; i < section_size_y; i++) {
        for (j = 0; j < section_size_x; j++) {
            ...
            //printf("lTid %d block(%d %d) - sTid %d - id(%d %d) - block(%d %d) | nIX %d - nIY %d | nTid %d | d_x_s[sTid] %f | d_x[nTid] %f | d_s_s[sTid] %f | d_s[nTid] %f\n",
            //    lTid, blockIdx.x, blockIdx.y, sTid, ix, iy, blockIdx.x, blockIdx.y, nIX, nIY, nTid, d_x_s[sTid], d_x[nTid], d_s_s[sTid], d_s[nTid]);
            if (nIX < (N + 2) && nIY < (N + 2))
...
void dens_step(dim3 grid, dim3 block, float *d_x, float *d_x0, float *d_u, float *d_v, float *d_densTemp) {
    ...
    //cudaFuncSetAttribute(diffuseOnGPU, cudaFuncAttributeMaxDynamicSharedMemorySize, 65536);
    ...
    for (int k = 0; k < 40; k++) { // inefficient -> multiple kernel calls
        diffuseOnGPU<<<grid, block, sizeShared + sizeSharedMax>>>(0, d_x, d_x0, d_densTemp, alpha, beta);
        SWAP(d_densTemp, d_x);
    }
...
void vel_step(dim3 grid, dim3 block, float *d_u, float *d_v, float *d_u0, float *d_v0, float *d_uTemp, float *d_vTemp) {
    ...
    //cudaFuncSetAttribute(diffuseOnGPU, cudaFuncAttributeMaxDynamicSharedMemorySize, 65536);
    //cudaFuncSetAttribute(computeDivergenceAndPressureOnGPU, cudaFuncAttributeMaxDynamicSharedMemorySize, 65536);
    ...
    */
    ...
    float beta = 1 + 4 * alpha;
    for (int k = 0; k < 40; k++) { // inefficient -> multiple kernel calls
        diffuseOnGPU<<<grid, block, sizeShared + sizeSharedMax>>>(1, d_u, d_u0, d_uTemp, alpha, beta);
        diffuseOnGPU<<<grid, block, sizeShared + sizeSharedMax>>>(2, d_v, d_v0, d_vTemp, alpha, beta);
        SWAP(d_uTemp, d_u);
        SWAP(d_vTemp, d_v);
    }
    computeDivergenceAndPressureOnGPU<<<grid, block, sizeSharedU + sizeSharedV>>>(d_u, d_v, d_u0, d_v0);

    alpha = 1;
    beta = 4;
    // d_u0 is p, d_v0 is div
    for (int k = 0; k < 40; k++) { // inefficient -> multiple kernel calls
        diffuseOnGPU<<<grid, block, sizeShared + sizeSharedMax>>>(0, d_u0, d_v0, d_uTemp, alpha, beta);
        SWAP(d_uTemp, d_u0);
    }
